...
figure;
m = mesh (double(repulsive),'FaceLighting','phong');
axis equal;
...
figure;
m = mesh (attractive,'FaceLighting','phong');
axis equal;
...
f = double(attractive + repulsive);

figure;
m = mesh (f,'FaceLighting','phong');
axis equal;
...
hold on;
p = mesh(sx, sy, sz,'FaceColor','red','EdgeColor','none','FaceLighting','phong');
hold off;
...
%     p.ZData = sz + f(P(2), P(1));
    hold on;
    p = mesh(sx + P(1), sy + P(2), sz + f(P(2), P(1)),'FaceColor','red','EdgeColor','none','FaceLighting','phong');
    hold off;
    pause(0.01)
end
